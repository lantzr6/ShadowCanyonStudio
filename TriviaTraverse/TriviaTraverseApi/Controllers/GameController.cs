...
namespace TriviaTraverseApi.Controllers
{
    public class GameController : ApiController
    {
        ...
        public IHttpActionResult JoinNewGame(int playerid, string gameType, string gameCap, bool gameAuto)
        {
            ...
            {
                case "5K":
                    stepCapValue = 5000; catMax = 4;
                    break;
                case "8K":
                    stepCapValue = 8000; catMax = 4;
                    break;
                case "10K":
                    stepCapValue = 10000; catMax = 4;
                    break;
            }
            ...
            //{
                ...
                vGame.VGamePlayers.Add(new VGamePlayer() { VGameId = vGame.VGameId, PlayerId = playerid, AutoRenew = gameAuto, LastStepUpdate = now, BotAvailableQuestions="", CreatedAt = now, UpdatedAt = now, Deleted = false });
                db.VGames.Add(vGame);
                db.SaveChanges();
            //}
            //for (int i = 1; i < 6; i++)
            ...
            //add bots
            List<Player> botPlayers = db.Players.Where(o => o.IsBot).OrderBy(r => Guid.NewGuid()).Take(4).ToList();
            foreach(Player bp in botPlayers)
            {
                vGame.VGamePlayers.Add(new VGamePlayer() { VGameId = vGame.VGameId, PlayerId = bp.PlayerId, AutoRenew = false, BotAvailableQuestions = "1,2,2,3,3,4,5", LastStepUpdate = now, CreatedAt = now, UpdatedAt = now, Deleted = false });
            }
            db.SaveChanges();
        ...
        public IHttpActionResult PostSelectedCategories(MobileVGameSelectedCategories objIn)
        {
            ...
            for (int i = 1; i <= objIn.SelectedCategories.Count(); i++)
            {
                VGamePlayerCategory playerVCat = new VGamePlayerCategory()
                {
                    PlayerId = objIn.PlayerId,
                    VGameCategoryId = objIn.SelectedCategories[i-1].VGameCategoryId,
                    SectionOrder = i,
                    CreatedAt = now,
                    UpdatedAt = now,
                    Deleted = false
